...
def main():
    print('=== Practice 6 Central Gateway: CAN File Sender ===')
    ...
    if chunk_count > 0xFFFF:
        print('File is too large for this training protocol.')
        return

    can_bus = CANInterface(channel=channel, bitrate=bitrate)
    can_bus.setup()
    try:
        can_bus.send(CAN_START_ID, struct.pack('>I', len(file_bytes)))
        ...
        print(f'Sent size information and {chunk_count} CAN chunks.')

        result = can_bus.receive_by_id(CAN_RESULT_ID, timeout=10)
        if result and result['data'][:1] == b'\x00':
            print('ECU size verification succeeded.')
        else:
            print('ECU size verification failed.')
    finally:
        can_bus.close()
