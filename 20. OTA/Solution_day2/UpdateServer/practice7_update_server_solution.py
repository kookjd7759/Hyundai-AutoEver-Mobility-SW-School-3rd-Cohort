...
def send_update_file(client, userdata):
    payload = {
        'type': 'update_file',
    ...
    print(
        f"File published for {userdata['target_ecu']} "
        f"(retry={userdata['retry_count']})."
    )
...
def on_connect(client, userdata, flags, reason_code):
    ...
    print('1. Update notice published.')
...
def on_message(client, userdata, message):
    ...
    if message.topic == RESPONSE_TOPIC:
        if payload.get('status') == 'accepted':
            print('2. Central Gateway accepted the update.')
            send_update_file(client, userdata)
    ...
    if payload.get('status') == 'success':
        print('7. ECU stored the update successfully.')
        userdata['finished'].set()
    elif payload.get('status') == 'failed':
        print('OTA path failed:', payload.get('reason'))
        if userdata['retry_count'] < userdata['max_retries']:
            userdata['retry_count'] += 1
            print('Retrying from Update Server to Central Gateway...')
            send_update_file(client, userdata)
        else:
...
def main():
    print('=== Practice 7 Update Server: End-to-End OTA ===')
    broker = input('Broker IP [localhost]: ').strip() or 'localhost'
    port = int(input('Broker Port [1883]: ').strip() or '1883')
    target_ecu = input('Target ECU (powertrain/body/adas) [powertrain]: ').strip() or 'powertrain'
